...
- **Block client behavior must NOT be registered from `<script>` tags inside `src/blocks/<name>/*.astro`.** Inline scripts inside the swup container don't execute on transitions. Register from `src/scripts/behaviors/<name>.ts` and lazy-import the impl. See `docs/scripts.md`.
...
```
src/
  blocks/           page-builder blocks (one folder per block + index.ts registry)
  ...
  scripts/
    core/           the kernel — DO NOT EDIT
    services/       global singletons (locomotive, swup, consent, analytics, transitions)
    behaviors/      DOM-selector behaviors (one folder OR one file per behavior)
    utils/          maths, string, viewport helpers
  stores/           nanostores ($mouse, $scroll, $screen, $consent, $deviceStatus)
  styles/           main.css + theme tokens + per-feature overrides
sanity.cli.ts       sanity CLI config (loads .env)
sanity.config.ts    studio config
astro.config.mjs    astro config
schema.json         extracted schema (regenerated by `bun run typegen`)
```
